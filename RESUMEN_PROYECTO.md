...
**Por qué importa:** al extraer `auth_service.py` en el Sprint 5, se descubrió un bug de seguridad que habría sido muy difícil de detectar en un router monolítico. La separación también hizo posible testear la lógica directamente sin levantar el servidor HTTP — lo que multiplicó la velocidad y cobertura de los tests.
...
**Situación:** `audit_service.registrar_auditoria()` no llama a `db.commit()`. Un auditor externo identificó esto como "bug crítico". Era un falso positivo.

**Explicación:** el log de auditoría se agrega a la sesión activa y el `commit()` lo hace el servicio que llama. Así el log y la operación principal se persisten en la **misma transacción** — si la operación falla, el log también hace rollback. Es diseño correcto, no bug.
...
**Problema:** `/cambiar-password` no usaba `get_current_user` de `deps.py` (intencional — para permitir el cambio cuando `debe_cambiar_password=True`). Pero al saltarse esa dependencia, también se perdía la validación de `session_token`.

**Consecuencia sin corrección:** un JWT revocado por logout o por login desde otro dispositivo podía usarse para cambiar la contraseña — account takeover potencial si el token fue interceptado.

**Solución:** extraer `session_id` del JWT en el router y pasarlo al servicio, que lo valida contra la BD. Se mantiene el bypass de `debe_cambiar_password` y se recupera la seguridad de sesión.
...
### 6.5 Cargo requiere area_id — constraint descubierto en tests

**Situación:** al escribir tests para `usuario_service`, el helper `make_cargo(db, empresa)` fallaba con `NOT NULL constraint failed: cargos.area_id`.

**Causa:** el modelo `Cargo` tiene `area_id` como FK obligatoria — un cargo siempre pertenece a un área. Esto no era obvio leyendo solo el servicio de usuario.

**Lección:** los tests descubren constraints del modelo que el código de producción asume silenciosamente. Los errores de FK en SQLite son más visibles que en PostgreSQL.

### 6.6 Propiedades ORM vs validators Pydantic para campos derivados

**Necesidad:** el frontend necesitaba `area_nombre` y `cargo_nombre` en la respuesta de usuarios, pero el modelo solo tiene `area_id` y `cargo_id`.

**Alternativas evaluadas:**
- `@model_validator` en Pydantic — funciona pero es verbose
- `@computed_field` en Pydantic — requiere que el campo fuente esté en el schema
- `@property` en el modelo SQLAlchemy — limpio, reutilizable, compatible con `from_attributes=True`

**Decisión:** agregar `area_nombre` y `cargo_nombre` como `@property` al modelo `User`. Pydantic con `from_attributes=True` los lee automáticamente. Consistente con el patrón que ya usaba el modelo `Cargo`.

### 6.7 Mockear servicios externos en tests

**Patrón establecido:** los tests nunca llaman a servicios externos reales. Se mockean con `unittest.mock.patch`:

```python
# Recaptcha en tests de auth
with patch("app.services.auth_service.validar_recaptcha", return_value=True):
    resultado = await auth_service.login(...)

# Correo en tests de usuario
with patch("app.services.usuario_service.enviar_correo_bienvenida", return_value=False):
    empleado = usuario_service.create_user(...)
```

El mock de correo fallido (`return_value=False`) es especialmente útil para verificar que el sistema no explota cuando el correo falla — el usuario se crea igual y solo se loguea un warning.
...
Los módulos con menor cobertura que aún pueden mejorar:

| Módulo | Cobertura | Dificultad | Recomendación |
|--------|-----------|-----------|---------------|
| `email_service` | 32% | Alta — requiere Resend real o mock complejo | Mock de la librería Resend |
| `deps.py` | 38% | Media — requiere JWT válidos en tests HTTP | Tests de integración con client fixture |
| `ai_service` | 44% | Alta — requiere Gemini API | Mock de la respuesta de Gemini |

### 7.2 Mejoras técnicas recomendadas

**Paginación en respuesta:** los endpoints de listado aceptan `skip` y `limit` pero no devuelven metadatos de paginación (`total`, `pagina_actual`, `total_paginas`). El frontend los necesita para construir paginadores.

**Soft delete consistente:** algunos modelos no tienen campo `activo`. Estandarizar para que ninguna entidad se elimine físicamente.

**Refresh token rotation:** al usar el refresh token, generar uno nuevo e invalidar el anterior. Esto previene ataques de token replay.

**Rate limiting por endpoint:** actualmente el rate limiting es global por usuario. Se podría afinar por endpoint (por ejemplo, `/auth/login` más restrictivo que `/incidentes/`).

### 7.3 Escalabilidad

**Base de datos:** Neon PostgreSQL serverless escala automáticamente. Para volúmenes altos, agregar índices en `fecha_creacion` de incidentes y auditorías.

**Caché:** los KPIs y el dashboard se calculan en tiempo real con queries. Para empresas con miles de incidentes, se podría cachear el resultado por empresa durante 5-10 minutos con Redis.

**Workers async:** las generaciones de PDF y Excel se hacen síncronamente en el request. Para archivos grandes, moverlas a un worker en background (Celery o FastAPI BackgroundTasks).

### 7.4 Para el equipo de frontend

- El endpoint `GET /usuarios/` ya devuelve `area_nombre` y `cargo_nombre` — no necesitan resolver IDs adicionales
- El parámetro `?activo=true|false` filtra usuarios; sin parámetro devuelve todos (incluyendo inactivos)
...
- La documentación interactiva está en `/docs` cuando `ENVIRONMENT=development`

### 7.5 Mantenimiento

- **Dependabot** corre semanalmente y abre PRs automáticos para actualizar dependencias con CVEs
- **Pre-commit hooks** garantizan que el código formateado llegue al repositorio
- **GitHub Actions** corre en cada push — si los tests fallan, el CI lo indica
- Antes de cada deploy, correr `.\venv\Scripts\python.exe -m pytest` para confirmar que los 176 tests pasan
