...
def generar_furat(db: Session, incidente_id: UUID, empresa_id: UUID) -> bytes:
    ...
    elementos.append(Paragraph(
        "FORMULARIO ÚNICO DE REPORTE DE ACCIDENTE DE TRABAJO",
        style_title
    ))
    elementos.append(Paragraph(
        "FURAT — Resolución 0156 de 2005 — República de Colombia",
        style_subtitle
    ))
    ...
    elementos.append(Paragraph("<b>SECCIÓN 2 — DATOS DEL TRABAJADOR ACCIDENTADO</b>", style_normal))

    trabajador = incidente.trabajador_afectado
    nombre_trabajador = trabajador.nombre if trabajador else "No especificado"
    cargo_trabajador  = trabajador.cargo.nombre if trabajador and trabajador.cargo else "No especificado"
    area_trabajador   = trabajador.area.nombre  if trabajador and trabajador.area  else "No especificado"
    ...
    tabla_trabajador = Table(datos_trabajador, colWidths=[3.5*cm, 6*cm, 3.5*cm, 6*cm])
    ...
    datos_accidente = [
        ...
        ["Tipo de evento", incidente.tipo.value,
         "Severidad", incidente.severidad.value],
        ["Descripción", incidente.descripcion, "", ""],
    ]

    tabla_accidente = Table(datos_accidente, colWidths=[3.5*cm, 6*cm, 3.5*cm, 6*cm])
    tabla_accidente.setStyle(TableStyle([
        ('GRID', (0,0), (-1,-1), 0.5, colors.black),
        ('BACKGROUND', (0,0), (0,-1), colors.lightgrey),
        ('BACKGROUND', (2,0), (2,1), colors.lightgrey),
        ('FONTSIZE', (0,0), (-1,-1), 8),
    ...
    tabla_lesion.setStyle(TableStyle([
        ('GRID', (0,0), (-1,-1), 0.5, colors.black),
        ('BACKGROUND', (0,0), (0,-1), colors.lightgrey),
        ('BACKGROUND', (2,0), (2,-1), colors.lightgrey),
        ('FONTSIZE', (0,0), (-1,-1), 8),
    ...
    elementos.append(Paragraph("<b>SECCIÓN 5 — INVESTIGACIÓN DE CAUSAS</b>", style_normal))

    investigacion = incidente.investigacion
    datos_inv = [
        ["Causas inmediatas",
         investigacion.causas_inmediatas if investigacion else "Pendiente de investigación"],
        ["Causas básicas",
         investigacion.causas_basicas if investigacion else "Pendiente de investigación"],
