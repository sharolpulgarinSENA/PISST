...
def create_incidente(db: Session, datos: IncidenteCreate,
                     empresa_id: UUID, reportado_por_id: UUID):
...
def update_estado_incidente(db: Session, incidente_id: UUID,
...
def create_investigacion(db: Session, incidente_id: UUID,
                         empresa_id: UUID, datos: InvestigacionCreate):
...
def create_accion_correctiva(db: Session, incidente_id: UUID,
                              empresa_id: UUID, datos: AccionCorrectivaCreate):
...
def update_accion_correctiva(db: Session, accion_id: UUID,
                              empresa_id: UUID, datos: AccionCorrectivaUpdate):
    """
    Actualiza una acción correctiva.
    No permite cerrarla sin evidencia documentada.
    """
    accion = db.query(AccionCorrectiva).filter(
...
def get_progreso_incidente(db: Session, incidente_id: UUID, empresa_id: UUID):
    ...
    total = db.query(AccionCorrectiva).filter(
    ...
    completadas = db.query(AccionCorrectiva).filter(
        AccionCorrectiva.incidente_id == incidente_id,
